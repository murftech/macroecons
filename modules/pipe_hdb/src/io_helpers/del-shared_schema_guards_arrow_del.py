...
def assert_overwrite_keys_exist(arrow_table: pa.Table, keys: list):
    ...
    # arrow, spark split
    missing = set(keys) - set(arrow_table.column_names)
    if missing:
        raise Exception(
...
def check_overwrite_keys_nullable_false(arrow_table: pa.Table, keys: list):
    """Check 1 - collects null-overwrite-key problems, and casts every overwrite key
    ...
    for col in keys:
        ...
        if null_count > 0:
            key_cols_with_null_values[col] = null_count
...
def reconcile_missing_columns(
    ...
    new_names: set,
    on_missingcols: str):
    """Check 3. Returns (arrow_table, new_names, problem_collect)."""
