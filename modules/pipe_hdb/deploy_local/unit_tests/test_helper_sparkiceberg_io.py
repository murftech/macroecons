...
Spark reimplementation of shared_schema_guards_arrow.py's shape/vocabulary).
...
from lakehouse_io.helper_sparkiceberg_io import (
    literal_is_valid,
...
def test_summarize_partitions_empty_batch_says_so(spark, capsys):
    ...
    out = capsys.readouterr().out
    assert out.strip() == 'Summaries:\n[spark] incoming batch has 0 rows - no partitions to replace.'
