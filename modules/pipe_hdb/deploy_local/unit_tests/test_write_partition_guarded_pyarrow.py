...
from lakehouse_io.helper_pyarrow_io import write_partition_guarded
...
# a bare string would otherwise reach assert_overwrite_keys_exist's set(keys) below and
# silently iterate character-by-character instead of raising cleanly)

def test_partition_keys_as_bare_string_raises(tmp_path, data_narrow):
    with pytest.raises(Exception, match='Please pass list'):
        write_partition_guarded(data_narrow, str(tmp_path / 't1'), 'tx_monthdate')


def test_partition_keys_as_multi_column_list_is_accepted(tmp_path, data_narrow):
    # parquet/iceberg genuinely support >1 partition column - a first write to an
    # empty dir just confirms the list shape itself is accepted, nothing more.
    write_partition_guarded(data_narrow, str(tmp_path / 't1'), ['tx_monthdate', 'town'])
