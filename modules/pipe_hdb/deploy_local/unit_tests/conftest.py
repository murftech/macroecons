...
lakehouse_io/ and reusable_functions/ are real packages (__init__.py, since the
2026-09-24 Tier 1 conversion) reachable via dotted imports (`from lakehouse_io
import helper_pyarrow_io`) once src/ itself is on sys.path - so this conftest's
only job besides fixtures is finding src/ and putting it there once, for every
test file in this directory, regardless of where this directory itself lives.
"""
import glob
import os
import sys
...
# unit_tests/ has moved twice now (src/unit_tests/ -> deploy_local/unit_tests/ ->
# src/unit_tests/ -> deploy_local/unit_tests/, all 2026-09-24) - a fixed parent-count
# broke silently on every move instead of erroring. Same self-correcting climb-to-anchor
# idiom deploy_databricks/*.sh and run.sh/Makefile already use for `.git`, just anchored
# on pyproject.toml + a sibling src/ (this module's own root) instead, since conftest.py
# doesn't sit inside src/ any more and can't assume any fixed depth from it.
def _find_module_src():
    for ancestor in Path(__file__).resolve().parents:
        if (ancestor / 'pyproject.toml').is_file() and (ancestor / 'src').is_dir():
            return ancestor / 'src'
    raise RuntimeError(f"conftest: no ancestor with pyproject.toml + src/ found above {__file__}")


_SRC = _find_module_src()
