...
cd "$(dirname "$0")"
# was a fixed "../.." - broke 2026-09-24 when this script moved one level deeper
# (modules/pipe_hdb/run.sh -> modules/pipe_hdb/deploy_local/run.sh): a hardcoded
# depth silently pointed at the wrong directory instead of erroring. Same
# self-correcting climb-to-.git idiom deploy_databricks/*.sh already uses, so a
# future move doesn't re-break this the same way.
...
case "$CALL_TASK" in
  land)    $RUN $SRC_DIR/pipeline/0_land_csv.py "$@" ;;
  import)  $RUN $SRC_DIR/pipeline/1_import_to_t1.py "$@" ;;
  stage)   $RUN $SRC_DIR/pipeline/2_stage_to_t2.py "$@" ;;
  report)  $RUN $SRC_DIR/pipeline/2.a_report_firstbq.py "$@" ;;
# next handle report
  # persist) $RUN $SRC_DIR/pipeline/3_persist_outputs_wip.py "$@" ;;
  ...
  all)     $RUN $SRC_DIR/pipeline/0_land_csv.py
           $RUN $SRC_DIR/pipeline/1_import_to_t1.py
           $RUN $SRC_DIR/pipeline/2_stage_to_t2.py ;;
