...
if [ -n "${RUNNING_IN_CONTAINER:-}" ]; then
  ...
  RUN="uv run --project modules/pipe_hdb python"
fi

STEP="${1:-all}"
case "$STEP" in
  ingest)  $RUN modules/pipe_hdb/0_import_datagov.py ;;

  report)  $RUN modules/pipe_hdb/2_report_firstbq.py ;;

  persist) $RUN modules/pipe_hdb/3_persist_outputs.py ;;

  all)     $RUN modules/pipe_hdb/0_import_datagov.py
           $RUN modules/pipe_hdb/2_report_firstbq.py
           $RUN modules/pipe_hdb/3_persist_outputs.py ;;

  *) echo "usage: ./run_pipeline.sh [ingest|report|persist|all]" >&2; exit 1 ;;

esac

# $RUN modules/pipe_hdb/0_import_datagov.py
# $RUN modules/pipe_hdb/2_report_firstbq.py
# $RUN modules/pipe_hdb/3_persist_outputs.py
